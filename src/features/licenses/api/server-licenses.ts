...
export const serverLicensesApi = {
  ...
  getLicenses: async (filters: FilterLicensesDto): Promise<LicenseResponse> => {
    ...
    const response = await serverAxios.get(`/licenses?${params.toString()}`);
    return response.data.data;
  },

  // Get license statistics
  getLicenseStats: async (): Promise<LicenseStats> => {
    const response = await serverAxios.get('/licenses/stats');
    return response.data.data;
  },

  // Get license by ID
  getLicense: async (id: string): Promise<License> => {
    const response = await serverAxios.get(`/licenses/${id}`);
    return response.data.data;
  },

  // Create new license
  createLicense: async (data: CreateLicenseDto): Promise<License> => {
    const response = await serverAxios.post('/licenses', data);
    return response.data.data;
  },

  // Assign license by email
  assignLicense: async (data: AssignLicenseDto): Promise<License> => {
    const response = await serverAxios.post('/licenses/assign', data);
    return response.data.data;
  },
  ...
  updateLicense: async (
    ...
    const response = await serverAxios.patch(`/licenses/${id}`, data);
    return response.data.data;
  },

  // Activate license
  activateLicense: async (id: string): Promise<License> => {
    const response = await serverAxios.patch(`/licenses/${id}/activate`);
    return response.data.data;
  },

  // Deactivate license
  deactivateLicense: async (id: string): Promise<License> => {
    const response = await serverAxios.patch(`/licenses/${id}/deactivate`);
    return response.data.data;
  },

  // Suspend license
  suspendLicense: async (id: string): Promise<License> => {
    const response = await serverAxios.patch(`/licenses/${id}/suspend`);
    return response.data.data;
  },

  // Revoke license
  revokeLicense: async (id: string): Promise<License> => {
    const response = await serverAxios.patch(`/licenses/${id}/revoke`);
    return response.data.data;
  },

  // Delete license
  deleteLicense: async (id: string): Promise<void> => {
    await serverAxios.delete(`/licenses/${id}`);
  }
};
